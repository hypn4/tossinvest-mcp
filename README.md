...
> **English**: MCP server exposing the Toss Securities (토스증권) Open API as tools for Claude and other AI agents. Covers Korean stock market (KRX) quotes, holdings, order execution, and a session-aware technical-analysis dashboard (MACD/RSI/Bollinger/VWAP). Read-only by default; opt-in trading. (Korean docs below.)
